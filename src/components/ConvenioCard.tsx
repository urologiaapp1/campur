...
export default function ConvenioCard({ convenio }: Props) {
  ...
  return (
    <Link href={`/convenio/${convenio.id}`} className="group block bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow border border-gray-100 overflow-hidden">
      ...
      <div className="p-4">
        {convenio.category && (
          <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full mb-2" style={{ backgroundColor: convenio.category.color + '20', color: convenio.category.color }}>
            {convenio.category.icon} {convenio.category.name}
          </span>
        )}
